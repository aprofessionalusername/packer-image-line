...
build {
  ...
  provisioner "shell" {
    inline = ["sleep 20 && wait $(pgrep ruby) && sleep 5 && wait $(pgrep ruby) && zypper up -y && zypper install --allow-unsigned-rpm -y https://github.com/vmware/cloud-init-vmware-guestinfo/releases/download/v1.1.0/cloud-init-vmware-guestinfo-1.1.0-1.el7.noarch.rpm"]
  }

  provisioner "shell" {
    inline = ["cloud-init clean"]
  }
